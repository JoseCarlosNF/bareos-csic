...
if [ ! -f /etc/bareos/bareos-config.control ]; then
  ...
  if [ "${WEBHOOK_NOTIFICATION}" = true ]; then
    sed -i '4c\  mailcommand = "/usr/local/bin/webhook-notify *Job*: %n - *JobID*: %i - *Tipo*: %t - *Nivel*: %l - *Cliente*: %c - *Status*: %e"' \
      /etc/bareos/bareos-dir.d/messages/Daemon.conf
    sed -i '5c\  mailcommand = "/usr/local/bin/webhook-notify *Job*: %n - *JobID*: %i - *Tipo*: %t - *Nivel*: %l - *Cliente*: %c - *Status*: %e"' \
      /etc/bareos/bareos-dir.d/messages/Standard.conf
    sed -i '4c\  operatorcommand = "/usr/local/bin/webhook-notify *Job*: %n - *JobID*: %i - *Tipo*: %t - *Nivel*: %l - *Cliente*: %c - *Status*: %e"' \
      /etc/bareos/bareos-dir.d/messages/Standard.conf
  fi
...
# Set Timezone
cp /usr/share/zoneinfo/$TZ /etc/localtime && echo -n $TZ >  /etc/timezone
